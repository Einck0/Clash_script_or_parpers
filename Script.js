...
const dnsConfig = {
    "enable": true,
    // "ipv6": true,
    // "default-nameserver": ["8.8.8.8", "223.5.5.5","114.114.114.114","system"], //默认 DNS, 用于解析 DNS 服务器 的域名
    "direct-nameserver": [...domesticNameservers], //用于direct出口域名解析的 DNS 服务器
    ...
    // "nameserver": domesticNameservers, //默认的域名解析服务器
    // "fallback": foreignNameservers, //后备域名解析服务器，一般情况下使用境外 DNS, 保证结果可信

    "nameserver": foreignNameservers, //默认的域名解析服务器
    // "fallback": foreignNameservers, //后备域名解析服务器，一般情况下使用境外 DNS, 保证结果可信
    ...
    "proxy-server-nameserver":['223.5.5.5',"1.1.1.1","114.114.114.114","https://doh.pub/dns-query"],
};
...
function trasformer(params){
  params.dns = dnsConfig;//修改dns
  proxyTrasformer(params);//修改代理方式和规则
  params["secret"] = "123123123";
  return params;
}
...
const rules = [
  ...
  "DOMAIN,cf.trackerslist.com,选择节点",
  ...
  "DOMAIN,yuc.wiki,香港",
  "DOMAIN,www.aigptx.top,DIRECT",
  "DOMAIN,cn2us02.opapi.win,DIRECT",
  "DOMAIN,c-z0-api-01.hash070.com,DIRECT",
...
//代理名称过滤
  ...
  const regions = [
    {
      name: "Special",
      regex: /自动|故障|流量|官网|套餐|机场|订阅|家宽|正|客服|限/,
    },
  ...
  params["rules"] = rules;
  console.log[params["proxy-groups"][8]];
  return params;
}
